...
from dataset import PGCNDataSet, collate_fn
...
def main():
    ...
    if (not args.evaluate) and (not args.debug):
        ...
        recorder = Recorder(args.snapshot_pref, ["exps", "__pycache__"])
    ...
    val_dataset = PGCNDataSet(dataset_configs, graph_configs,
    ...
    model = PGCN(model_configs, graph_configs, dataset_configs)
...
def train(train_loader, model, act_criterion, comp_criterion, regression_criterion, optimizer, epoch):
    ...
    for i, (batch_act_fts, batch_comp_fts, batch_props_type,
            batch_props_label, batch_reg_target, props_num_list, vid_list) in enumerate(train_loader):
        ...
        regression_out, regression_labels, regression_target, ohem_num = model(batch_act_fts, batch_comp_fts,
                                                                               batch_props_type,
                                                                               batch_props_label, batch_reg_target,
                                                                        props_num_list, vid_list, True)
        try:
            act_loss = act_criterion(activity_out, activity_target)
        except:
            a = 0
        comp_loss = comp_criterion(completeness_out, completeness_target, ohem_num, 7 * ohem_num)
        ...
        fg_indexer = (activity_prop_type == 0).nonzero().squeeze()\
            if len((activity_prop_type == 0).nonzero()) != 1 else (activity_prop_type == 0).nonzero().squeeze(-1)
        bg_indexer = (activity_prop_type == 2).nonzero().squeeze()\
            if len((activity_prop_type == 2).nonzero()) != 1 else (activity_prop_type == 2).nonzero().squeeze(-1)
...
def validate(val_loader, model, act_criterion, comp_criterion, regression_criterion, iter):
    ...
    for i, (batch_act_fts, batch_comp_fts, batch_props_type,
            batch_props_label, batch_reg_target, props_num_list, vid_list) in enumerate(val_loader):
        # measure data loading time
        batch_size = batch_act_fts.size(0)
        ...
        regression_out, regression_labels, regression_target, ohem_num = model(batch_act_fts, batch_comp_fts, batch_props_type,
                                                                     batch_props_label, batch_reg_target, props_num_list,
                                                                     vid_list, False)
