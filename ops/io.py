...
def load_proposal_file(filename, mode='Train'):
    ...
    def parse_group_train(info):
        vid = info[0]
        n_frame = info[1]
        w_start, w_end = info[2].strip().split()
        w_start = int(w_start)
        w_end = int(w_end)
        ...
        offset += n_gt
        n_props = int(info[offset])
        offset += 1
        props_boxes = [x.split() for x in info[offset: offset + n_props]]

        return vid, n_frame, gt_boxes, props_boxes, (w_start, w_end)

    def parse_group_test(info):
        offset = 0
        vid = info[offset]
        offset += 1
    ...
    parse_results = [parse_group_test(l) for l in info_list] if mode != 'Train' else [parse_group_train(l) for l in info_list]
